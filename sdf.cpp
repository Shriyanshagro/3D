...
void createobstacle ()
{
  ...
  static const GLfloat vertex_buffer_data2 [] = {
       ...
       -0.02f, 0.02f, 0.02f, // triangle 1 : end

       0.02f, 0.02f,-0.02f, // triangle 2 : begin

       -0.02f,-0.02f,-0.02f,

       -0.02f, 0.02f,-0.02f, // triangle 2 : end
  ...
  for(int r=1;r<=num_obs;r++)
  {
      ...
      if(appear[r]%(num_obs/2)==0){
          coins[coin_count] = create3DObject(GL_TRIANGLES, 36, vertex_buffer_data2, color_buffer_data2, GL_FILL);
          coin_count++;
      }
  }
}
...
void draw ()
{
  ...
  int count_coin =0 ;
  glm::mat4 translateobstacle[num_obs+1],obstacle_rotation[num_obs+1],rotateobstacle[num_obs+1],translatecoin[coin_count+1],rotatecoin[coin_count+1];
  for(int r=1;r<=num_obs;r++)
  {
      Matrices.model = glm::mat4(1.0f);
    ...
    if(appear[r]%(num_obs/2)==0){
        ...
        if(visibility[r]>appear_time)
            visibility[r]=1;
        translatecoin[count_coin] = glm::translate (glm::vec3(obsx[count_coin],botpos[2]-0.12f+mov[count_coin]+0.05f,obsz[count_coin]));        // glTranslatef
        rotatecoin[count_coin] = glm::rotate((float)(180*M_PI/180.0f), glm::vec3(0,1,0)); // rotate about vector (-1,1,1)
        Matrices.model *= (rotatecoin[count_coin]*translatecoin[count_coin] );
        MVP = VP * Matrices.model;
        glUniformMatrix4fv(Matrices.MatrixID, 1, GL_FALSE, &MVP[0][0]);

        draw3DObject(coins[count_coin++]);
...
int main (int argc, char** argv)
{
    ...
    // buffer1.loadFromFile("car_x.wav");
    //     // return -1;
